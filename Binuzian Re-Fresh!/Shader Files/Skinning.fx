...
VS_OUTPUT vs_Skinning(VS_INPUT_SKIN IN)
{
	...
	for (int i = 0; i < MaxNumAffectingBones - 1; i++)
	{
		...
		v += IN.weights * mul(IN.position, BoneMatrices[IN.boneIndices[i]]);
		norm += IN.weights * mul(IN.normal, BoneMatrices[IN.boneIndices[i]]);
		//Sum up the weights
